...
def main():
	...
	if maxAlphabet:
		refDists=[[1/m if i<m else 0 for i in range(len(alphabet))] for m in range(1,maxAlphabet+1)]
	else:
		refDists=[[1/m if i<m else 0 for i in range(len(alphabet))] for m in range(1,len(alphabet)+1)]
	IUPACdict={"AG":"R","CT":"Y","CG":"S","AT":"W","GT":"K","AC":"M","CGT":"B","AGT":"D","ACT":"H","ACG":"V","ACGT":"N"}
...
def parseMeme(inData):
	...
	alphabet = ''.join([line.lstrip("ALPHABET=").strip() for line in header.split("\n") if re.match("ALPHABET*", line)])
...
def rates2kmer_max(L,alphabet):
	sList, sIndex = list(zip(*sorted([(l, i) for i, l in enumerate(L)], reverse=True)))
...
def indMostInfo(L, refDists, penalty):
	sList, sIndex = list(zip(*sorted([(l, i) for i, l in enumerate(L)], reverse=True)))
	jsds = [ [jsd(sList, refDist) + penalty*penalty*(i+1), i+1] for i, refDist in enumerate(refDists) ]
	keepN = min(jsds)[1]
	return sIndex[:keepN]
...
def indMSE(L, refDists, penalty):
	sList, sIndex = list(zip(*sorted([(l, i) for i, l in enumerate(L)], reverse=True)))
	SEs = [ [SE(sList, refDist) + penalty*penalty*(i+1), i+1] for i, refDist in enumerate(refDists) ]
	keepN = min(SEs)[1]
	return sIndex[:keepN]
